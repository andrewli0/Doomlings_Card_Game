...
const HOVERED_CARD_SCALE = 0.9 # make 1 so can read the card
...
func add_card_to_hand(card, speed):
	...
	if card not in player_hand: # case 1
		print("PlayerHand.gd player_hand: ", player_hand)
		player_hand.insert(player_hand.size(), card) # insert card into last element of array
...
func update_hand_positions(speed):
	# print("Called update_hand_positions(), hand size: ", player_hand.size())
	for i in range(player_hand.size()):
		# get new card position based on how many cards in hand
		# card should be drawn to bottom of screen
		var HAND_Y_POSITION = get_viewport().get_visible_rect().size.y*0.85
		var new_position = Vector2(calculate_card_position(i), HAND_Y_POSITION)
		var card = player_hand[i] # card we want to move
		print("check")
...
func calculate_card_position(index):
	...
	var x_offset = (player_hand.size() - 1) * CARD_WIDTH # video names var total_width
