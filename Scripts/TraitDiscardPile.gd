...
var trait_discard_pile_deck = [] # trait discard pile
# var trait_card_database_reference


var player_hand_reference
var card_manager_reference
var enemy_one_hand_reference
var timer
...
func _ready() -> void:
	...
	# player_trait_deck.shuffle() # suffles deck
	# print("Area 2D collision mask deck: ", $Area2D.collision_mask) 
	$RichTextLabel.text = str(trait_discard_pile_deck.size()) # cast int to string. Set counter to number of cards in deck 
	player_hand_reference = $"../PlayerHand"
	card_manager_reference = $"../Card Manager"
	enemy_one_hand_reference = $"../Enemy1Hand"
	# trait_card_database_reference = preload("res://Scripts/TraitCardDatabase.gd")
...
func draw_card(turn, play_animation):
	...
	print ("Drawing Card Trait Discard Deck")
	wait(1.0)
	if  trait_discard_pile_deck.size() == 0:
		...
		var card_drawn_name = trait_discard_pile_deck[0]
		
		trait_discard_pile_deck.erase(card_drawn_name)
		$RichTextLabel.text = str(trait_discard_pile_deck.size())
...
func add_card_to_top(card):
	# print("add ", card, " to trait discard pile top")
	...
	trait_discard_pile_deck.insert(0, card_name_without_numbers) # player deck only stores names
	...
	$RichTextLabel.text = str(trait_discard_pile_deck.size()) # cast int to string
