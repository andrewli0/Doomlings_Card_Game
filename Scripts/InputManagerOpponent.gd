...
func _ready() -> void:
	...
	trait_deck_reference = get_tree().get_root().get_node("Main/PlayerField/TraitDeck")
	age_deck_reference = get_tree().get_root().get_node("Main/PlayerField/AgeDeck")
	trait_discard_pile_reference = get_tree().get_root().get_node("Main/PlayerField/TraitDiscardPile")
	age_discard_pile_reference = get_tree().get_root().get_node("Main/PlayerField/AgeDiscardPile")
	player_hand_reference = get_tree().get_root().get_node("Main/PlayerField/PlayerHand")
...
func raycast_at_cursor():
	...
	if result.size() > 0:
		...
		if result_collision_mask == COLLISION_MASK_CARD: 
			...
			# trait_deck_reference.draw_card(USER)
			print("InputManagerOpponentScript: deck clicked")
			trait_deck_reference.deck_clicked()
			...
			age_deck_reference.draw_card(USER)
